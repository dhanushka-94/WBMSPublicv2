                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="p-6 border-b border-gray-200">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <h4 class="text-lg font-semibold text-gray-900">{{ $reading->waterMeter && $reading->waterMeter->customer ? $reading->waterMeter->customer->full_name : 'Unassigned Customer' }}</h4>
                                    <p class="text-sm text-gray-500">{{ $reading->waterMeter && $reading->waterMeter->customer ? $reading->waterMeter->customer->account_number : 'No Account' }}</p>
                                </div>
                            </div>
                        </div>
                ...
                <form action="{{ route('readings.update', $reading) }}" method="POST" id="editReadingForm" @if($reading->status === 'billed') class="pointer-events-none opacity-60" @endif>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <select name="water_meter_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                                <option value="">Select a Water Meter</option>
                                @foreach($waterMeters as $meter)
                                    <option value="{{ $meter->id }}" {{ $meter->id == $reading->water_meter_id ? 'selected' : '' }}>
